...
import { CreateComposerSitePayload, IPC_EVENTS } from './constants';
import { findPhpBinFallback, runComposer } from './composer';
import { ensureEnvFromExample, findEnvFile, setEnvValue } from './envFix';
import { getProject, invalidateProject } from './project';
import { webRootDirOf } from './webroot';
...
export const fetchText = (source: string, redirects = 0): Promise<string> => {
	if (source.startsWith('file://')) {
		return fetchText(source.slice('file://'.length));
	}
	...
	return new Promise((resolve, reject) => {
		...
		get(source, (res) => {
			if (res.statusCode && res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
				...
				resolve(fetchText(new URL(res.headers.location, source).toString(), redirects + 1));
				return;
			}
 ...
 * `…/app/public_html`.
 */
export const retargetDocumentRoot = (sitePath: string, webRootDir: string): string[] => {
	const defaultWebRoot = path.join(sitePath, 'app', 'public');
	const pattern = new RegExp(
		defaultWebRoot.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '(?![\\w-])',
		'g',
	);
...
export default function registerAddSiteComposer(context: LocalMain.AddonMainContext, logger: any): void {
	const {
		addSite,
		siteData,
		wpCli,
	...
	const freshSite = (siteId: string) => new Local.Site(siteData.getSite(siteId));

	const message = (siteId: string, label: string) => {
		LocalMain.sendIPCEvent('updateSiteMessage', siteId, label ? { label, stripes: true } : '');
	};

	const banner = (siteId: string, variant: 'success' | 'warning' | 'error', title: string, body?: string) => {
	...
	const socketFor = (site: any): string => {
		try {
	...
	const validateTemplate = async (templateUrl: string): Promise<string> => {
		const templateRaw = (await fetchText(templateUrl)).replace(/^﻿/, '');
		let template: any;
		try {
	...
	const bootstrap = async (
		{ newSiteInfo, wpCredentials, templateUrl }: CreateComposerSitePayload,
		templateRaw: string,
	) => {
		...
		try {
			...
			const install = await runComposer(phpBin, appDir, ['install'], COMPOSER_INSTALL_TIMEOUT_MS);
			if (!install.ok) {
			...
			if (!project?.coreDir || !project.coreInstalled) {
				throw new Error(
					'composer install finished but no WordPress core dir was found — does the template declare extra["wordpress-install-dir"]?',
				);
			}

			message(siteId, 'Configuring .env');
			ensureEnvFromExample(project.dir);
			const envPath = findEnvFile(sitePath);
			if (envPath) {
				setEnvValue(envPath, 'DB_HOST', `localhost:${socketFor(site)}`, true);
				setEnvValue(envPath, 'DB_NAME', 'local');
				setEnvValue(envPath, 'DB_USER', 'root');
				setEnvValue(envPath, 'DB_PASSWORD', 'root');
				setEnvValue(envPath, 'WP_DOMAIN', site.domain);
				setEnvValue(envPath, 'MULTISITE', 'false');
			}

			message(siteId, 'Pointing the web server at the composer webroot');
			const webRootDir = webRootDirOf(project.dir, project.coreDir);
			retargetDocumentRoot(sitePath, webRootDir);
			...
			if (multiSite) {
				if (envPath) {
					...
					setEnvValue(envPath, 'MULTISITE', multiSite === Local.MultiSite.Subdomain ? 'domains' : 'true');
				}
				...
				try {
					...
					logger.warn(`Multisite domain sync after bootstrap failed: ${err?.message ?? err}`);
				}
			}
			...
			banner(
				siteId,
				'warning',
				'Composer bootstrap incomplete.',
				`${err?.message ?? err} — the site itself was created; you can finish the remaining steps manually.`,
			);
			logger.error(`Composer bootstrap failed: ${err?.message ?? err}`);
		}
	};

	LocalMain.addIpcAsyncListener(
		IPC_EVENTS.CREATE_COMPOSER_SITE,
		async (payload: CreateComposerSitePayload): Promise<{ ok: boolean; error?: string }> => {
