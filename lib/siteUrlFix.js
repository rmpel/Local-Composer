 ...
 * covers both consumers. When their exact siteurl lookup comes through for
 * a composer site, the known core subdir is stripped from the returned URL.
 ...
 * still repairable — only the legitimate core-path suffix stops doing so.
 */
/** The one query shape the banner check and the domain changer issue. */
const SITEURL_QUERY = /^SELECT option_value FROM \S+options WHERE option_name='siteurl' LIMIT 1$/;
const applySiteUrlQueryPatch = ({ siteDatabase, logger }) => {
    ...
    logger.info('SiteDatabaseService.runQuery patched: siteurl reports without the composer core subdir.');
    return true;
};
